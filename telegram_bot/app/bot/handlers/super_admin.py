    ...
    *,
    ...
    try:
        ...
        await message.answer(
            service.access_denied_message(current_user.language if current_user else None),
...
async def handle_companies(
    message: Message,
    session: AsyncSession,
    current_user: UserDTO | None,
) -> None:
    super_admin = await _get_super_admin(message, session=session, current_user=current_user)
    if super_admin is None or super_admin.language is None:
        return

    service = build_super_admin_service(session)
    await message.answer(
        service.companies_placeholder(super_admin.language),
        reply_markup=build_super_admin_keyboard(super_admin.language),
    )
...
async def handle_statistics(
    ...
    service = build_super_admin_service(session)
    await message.answer(
        service.statistics_placeholder(super_admin.language),
        reply_markup=build_super_admin_keyboard(super_admin.language),
    )
...
async def handle_settings(
    ...
    service = build_super_admin_service(session)
    await message.answer(
        service.settings_placeholder(super_admin.language),
        reply_markup=build_super_admin_keyboard(super_admin.language),
    )
