...
from db.models import Company, CompanyChatMessage, Request, RequestMessage, User  # noqa: F401
...
class Database:
    ...
    def _run_sqlite_migrations(connection) -> None:
        ...
        if "request_messages" not in table_names:
            connection.execute(
                text(
        ...
        connection.execute(
            text(
                "CREATE INDEX IF NOT EXISTS ix_request_messages_sender_user_id "
                "ON request_messages (sender_user_id)"
            )
        )

        if "company_chat_messages" not in table_names:
            connection.execute(
                text(
        ...
        connection.execute(
            text(
                "CREATE INDEX IF NOT EXISTS ix_company_chat_messages_company_id "
                "ON company_chat_messages (company_id)"
            )
        )
        connection.execute(
            text(
                "CREATE INDEX IF NOT EXISTS ix_company_chat_messages_channel_type "
                "ON company_chat_messages (channel_type)"
            )
        )
        connection.execute(
            text(
                "CREATE INDEX IF NOT EXISTS ix_company_chat_messages_sender_user_id "
                "ON company_chat_messages (sender_user_id)"
            )
        )
